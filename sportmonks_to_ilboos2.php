...
function is_choice_exists($id, $match_name) {
    global $link;
    if($id== Null){$id=0;}//fede
    $q = mysqli_query($link, 'SELECT * FROM ilboos_bp_bet_options WHERE bet_event_cat_id = ' . $id .' AND bet_option_name  = "' . $match_name . '"' );
    return $q->fetch_array();
}
...
function betpress_get_cats_max_order($bet_event_ID) {
    ...
    global $link;
    $q = mysqli_query($link, 'SELECT MAX(bet_event_cat_sort_order) FROM ilboos_bp_bet_events_cats WHERE bet_event_id ='. $bet_event_ID );
...
$cURL = curl_init();
$setopt_array = array(CURLOPT_URL => "https://soccer.sportmonks.com/api/v2.0/fixtures/between/" . $day_from . "/" . $day_to . "?api_token=qrGfkDWo2F4xOVgspVvpPxKVVWwuTO4jlch7TVOsDQC7bYLe0G49cZ5vs0P0&include=localTeam,visitorTeam,league,odds&filter[odds.bookmaker_id=2]&markets=1&leagues=384&tz=Europe/Rome",    CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPHEADER => array());
curl_setopt_array($cURL, $setopt_array);
$json_response_data = curl_exec($cURL);
//print_r($json_response_data);
curl_close($cURL);
$sportmonks = json_decode($json_response_data, true);
if($sportmonks['data'] == null){echo"non ci sono dati";
}
    foreach ($sportmonks['data'] as $sport) {//PER OGNI RIGA DEL FEED
        ...
        $bet_inserted_id =0;//INIZIALIZZO ID NEL DB ILBOOS DA BET_EVENTS
            foreach ($sport['league'] as $event['data']) {//PER OGNI RIGA DEL FEED SCORRI LE LEGHE E INSERISCI
           ...
           if (! is_match_exists($match_id)) {//INSERISCE LE PARTITE
              ...
              $match_start_date = $sport['time']['starting_at']['timestamp']  +0100;
                ...
                foreach ($sport['odds']['data'] as $bets) {
                    echo"<br>sono fico: "; echo $bets['name'];
                  
                    $bets_id = $bets['id'];
                    foreach ($bets as $bet_ID) {
                    //$bet_id = $bets['id'];
                        //$bet_id = $bet_ID;
                        echo"ecco qui: ";echo"<br>";echo $bets_id;echo"<br>";
                        ...
                        if (! is_choice_exists($choice_id, $bet_name)) {
                            foreach ($choice['odds']['data'] as $option) {
                                    ...
                                    if (! is_bet_exists($bets_id, $match_id)) {
                                        ...
                                        $insert_bet = mysqli_query($link, 'INSERT INTO ilboos_bp_bet_events_cats (bet_event_cat_sort_order, id_sm_match, bet_event_id, id_sportmonks, bet_event_cat_name) VALUES (' . ++ $bet_event_cats_max_sort_order['MAX(bet_event_cat_sort_order)'] . ',' . $match_id . ', ' . $match_inserted_id . ', ' . $bets_id . ', "' . $bet_name .'"  )');
                                        ...
                                        $q = mysqli_query($link, 'SELECT bet_event_cat_id FROM ilboos_bp_bet_events_cats WHERE bet_event_id = ' . $match_inserted_id . ' AND bet_event_cat_name = "' . $bet_name .'" ');
                                    ...
                                    $insert_choice = mysqli_query($link, 'INSERT INTO ilboos_bp_bet_options (bet_option_sort_order, bet_event_cat_id, bet_option_name, bet_option_odd, id_sportmonks, status, opt_winnings, opt_ammo, id_sm_match ) '
                                        . 'VALUES (' . ++ $bet_option_max_sort_order['MAX(bet_option_sort_order)'] . ' ,' . $bet_inserted_cat_id . ', "' . $choice_name . '", ' . $choice_odd . ', ' . $bet_inserted_id . ', "awaiting" , "0", "0",' . $match_id .' )');
